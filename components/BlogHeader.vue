...
<template>
	<header class="flex w-full justify-between bg-primary dark:bg-primary-dark p-2 dark:text-text-dark">
		...
		<h1 v-if="loggedIn" class="hidden md:flex justify-center justify-self-center items-center gap-1 cursor-default">
			<NuxtImg :src="profile.avatar" class="w-7 h-7 rounded-3xl border-accent border-2"/> {{ profile.username }}</h1>
